...
=end
	...
	def add_topics(s)
		...
		titles = []
		s.split(/\r|\n|\t/).each do |line|
			line.strip.scan(/^\d+\s(?<title>[A-Z][A-Za-z0-9\s\'\/\\\-]+)/) do |title|
				titles << title[0] unless exclude? title[0]
			end
			line.strip.scan(/^(?<title>[A-Z][A-Za-z0-9\s\'\/\\\-]+)\d+/) do |subtitle|
				titles << subtitle[0] unless exclude? subtitle[0]
			end
		end
	...
	def get_topics
		...
		if self.topics.blank?
			...
			s.scan(/Symbols\sand\nabbreviations/) do |match|
				topics_text ||= Regexp.last_match.pre_match
			end
			
			self.topics = add_topics(topics_text)
	...
	## here first you need all the text before symbols and abbreviations
